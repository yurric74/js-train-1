...
function isEvenNumber(num) {
   ...
   return num % 2 === 0;// Повернути результат перевірки (true або false) як результат функції
}
...
function calculateSum(n) {
  ...
  for (let i = 1; i<=n; i++){// Запустити цикл для змінної i від 1 до n
...
function isPrimeNumber(num) {
  if (num<=1) {return false;}// Якщо число num менше або дорівнює 1, то воно не є простим, тому повертаємо false
  // В іншому випадку продовжуємо перевірку
  for (let i = 2; i<=num-1; i++){// Запускаємо цикл для змінної i від 2 до num-1
  if (num%i===0) {return false;}// Перевіряємо, чи num ділиться націло на поточне значення i
  else true;}// Якщо так, то num не є простим, тому повертаємо false
...
function isPositiveNumber(num) {
  ...
  return num>0;// В іншому випадку, якщо num менше або дорівнює 0, то num не є позитивним, тому повертаємо false
}
...
function isEmptyString(str) {
  ...
  return str === '';// В іншому випадку, якщо str містить хоча б один символ, то str не є порожнім рядком, тому повертаємо false
}
...
function checkDataType(value) {
  const valueType = typeof value;// Отримуємо тип значення value за допомогою оператора typeof
   console.log(`Значення "${value}" має тип "${valueType}"`);// Виводимо у консоль повідомлення, яке містить значення value та його тип
...
function findMax(num1, num2) {
  if (num1>num2){// Порівнюємо num1 та num2
  return num1;}
  else  (num1<=num2)// Якщо num1 більше num2, то num1 є максимальним, тому повертаємо num1
  return num2;// В іншому випадку (якщо num2 більше або дорівнює num1), то num2 є максимальним, тому повертаємо num2
}
...
function getDayOfWeek(dayNumber) {
  ...
  let day;// В кожному випадку (case) перевіряємо значення dayNumber і присвоюємо змінній day відповідну назву дня тижня
  switch (dayNumber) {// Якщо dayNumber = 1, то day = "Понеділок"
    case 1: day = "Понеділок"; break;
    case 2: day = "Вівторок"; break;// Якщо dayNumber = 2, то day = "Вівторок"
    case 3: day = "Середа"; break;// Якщо dayNumber = 3, то day = "Середа"
    case 4: day = "Четвер"; break;// Якщо dayNumber = 4, то day = "Четвер"
    case 5: day = "П'ятниця"; break;// Якщо dayNumber = 5, то day = "П'ятниця"
    case 6: day = "Субота"; break;// Якщо dayNumber = 6, то day = "Субота"
    case 7: day = "Неділя"; break;// Якщо dayNumber = 7, то day = "Неділя"
    default: day = "Невірний номер дня";}// Якщо значення dayNumber не співпадає з жодним з випадків, то day = "Невірний номер дня"
   return day;// Повертаємо назву дня тижня, яку зберегли у змінній day
}
...
function getVariableType(variable) {
  const variableType = typeof variable;// Отримуємо тип змінної variable за допомогою оператора typeof
  const message =// Встановлюємо відповідне повідомлення залежно від отриманого типу
  variableType === "number"?"Це число":// Якщо тип змінної - number, то повідомлення = "Це число"
  variableType === "string"?"Це рядок":// Якщо тип змінної - string, то повідомлення = "Це рядок"
  variableType === "boolean"?"Це булеве значення":"Це інший тип";// Якщо тип змінної - boolean, то повідомлення = "Це булеве значення"
...
function factorial(n) {
  ...
  else {return n*factorial(n-1);}// Якщо n більше за 1, ми множимо n на результат виклику тієї самої функції з n-1.
}
...
function makeAdder(x) {
  return function(y) {// Вертаємо нову функцію, яка приймає `y`
...
function divide(x) {
  return function(y) {// Повертаємо нову функцію, яка приймає `y`
...
function memoizedArea() {
  let prevSide = null; let prevResult = null;// Ініціалізуємо примітивні змінні для зберігання попереднього аргументу та результату
  return function(side){// Повертаємо функцію яка приймає аргумент side - сторону квадрата
  if (prevSide===side) {// Перевіряємо, чи є введена сторона такою ж, як і в попередньому виклику
  console.log("Fetching from cache");// Виводимо повідомлення Fetching from cache
  return prevResult;}// Повертаємо попереднє значення
  else { // Якщо сторона відрізняється від попередньої, обчислюємо площу та зберігаємо результат та аргумент для майбутнього використання
  console.log("Calculating result");// Виводимо повідомлення Calculating result
  const result = side*side;// Розраховуємо площу
  prevSide=side;// prevSide присвоюємо сторону квадрата
  prevResult=result;// prevResult просвоюємо площу квадрата
...
function compose(func1, func2) {
  return function(n){// Повертаємо нову функцію, яка приймає аргумент n
  return func2(func1(n));};// Повертаємо func2, в яку передаємо func1 з аргументом n
}
